...
| **[review-prd](.claude/skills/review-prd/)** | PRD 공정 리뷰 — 독립 에이전트가 PRD를 검증 | `/review-prd` |
...
| **[investigate](.claude/skills/investigate/)** | 이슈/버그 코드 조사 — Jira 이슈 키·URL·키워드로 원인 분석 | `/investigate` |
...
| **[pre-commit-review](.claude/skills/pre-commit-review/)** | 커밋/푸시/PR 전 운영 안정성·사이드이펙트 점검 | 자동 트리거 |
| **[coverage-check](.claude/skills/coverage-check/)** | 패턴 커버리지 매트릭스 검증 — 모듈 간 일관성 체크 | `/coverage-check` |
...
| **[frontend-design-guide](.claude/skills/frontend-design-guide/)** | 프론트엔드 설계 4원칙 (가독성·예측가능성·응집성·결합도) | 자동 트리거 |
...
| **[prompt-rewrite](.claude/skills/prompt-rewrite/)** | 프롬프트 최적화 리라이터 — 승인 후 자동 실행 | 자동 트리거 |

### 프론트엔드 디자인 (third-party 스킬 팩)
...
```
① grill-me → ② write-prd → ②½ review-prd → ③ prd-to-issues → ④ dev-cycle
   (심문)       (PRD 작성)    (PRD 리뷰)       (이슈 분해)          (구현)
...
커밋/푸시/PR 시점: **pre-commit-review** 자동 트리거 → 운영 안정성 점검
